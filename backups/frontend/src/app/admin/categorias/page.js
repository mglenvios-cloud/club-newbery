...
export default function AdminCategorias() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Fallback local storage si el servidor falla
        const local = localStorage.getItem('jn-categories');
        setCategories(local ? JSON.parse(local) : defaultMockCategories);
      }
    } catch (e) {
      console.warn("Backend offline, cargando fallback local");
      const local = localStorage.getItem('jn-categories');
      setCategories(local ? JSON.parse(local) : defaultMockCategories);
    } finally {
      setLoading(false);
  ...
  const defaultMockCategories = [
    { id: 1, name: "Socio Activo", type: "SOCIO", price: 12000, description: "Mayores de 18 años con acceso total" },
    { id: 2, name: "Socio Cadete", type: "SOCIO", price: 8000, description: "Jóvenes de 13 a 17 años" },
    { id: 3, name: "Socio Infantil", type: "SOCIO", price: 6000, description: "Niños de 5 a 12 años" },
    { id: 4, name: "Futsal Primera", type: "DISCIPLINA", price: 15000, description: "Arancel mensual Futsal AFA" },
    { id: 5, name: "Patín Show", type: "DISCIPLINA", price: 11000, description: "Clases de patín artístico de competición" }
  ];
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      // Fallback
      console.warn("Utilizando guardado local alternativo:", e.message);
      const newCat = {
        id: Date.now(),
        ...payload
      };
      ...
      localStorage.setItem('jn-categories', JSON.stringify(updated));
      setSuccessMsg("¡Categoría creada de forma local (Offline)!");
      setName("");
      setPrice("");
      setDescription("");
    }
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      console.warn("Utilizando actualización local alternativo:", e.message);
      const updated = categories.map(c => c.id === editId ? { ...c, ...payload } : c);
      setCategories(updated);
      localStorage.setItem('jn-categories', JSON.stringify(updated));
      setSuccessMsg("¡Categoría actualizada de forma local (Offline)!");
      handleCancelEdit();
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      console.warn("Eliminación local offline para ID:", id);
