  ...
  const fetchPlayers = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const local = localStorage.getItem('jn-players');
        setPlayers(local ? JSON.parse(local) : defaultMockPlayers);
      }
    } catch (e) {
      console.warn("Backend offline, cargando perfiles inferiores locales");
      const local = localStorage.getItem('jn-players');
      setPlayers(local ? JSON.parse(local) : defaultMockPlayers);
    } finally {
      setLoading(false);
