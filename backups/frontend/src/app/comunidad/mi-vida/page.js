...
export default function MiVidaSocial() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setPosts(local ? JSON.parse(local) : defaultMockPosts);
      }
    } catch (e) {
      console.warn("Backend offline, cargando muro local");
      const local = localStorage.getItem('jn-posts');
      setPosts(local ? JSON.parse(local) : defaultMockPosts);
    } finally {
      setLoading(false);
  ...
  const handlePost = async (e) => {
    ...
    try {
      ...
      console.warn("Moderación y publicación local (Offline)");
      // Palabras prohibidas offline
      const prohibidas = ["mierda", "puto", "boludo", "pelotudo", "idiota"];
      const contieneOfensa = prohibidas.some(w => content.toLowerCase().includes(w));

      if (contieneOfensa) {
        setModWarning("⚠️ Newbery IA retuvo tu publicación: Contiene lenguaje inadecuado. Quedará pendiente de moderación.");
      } else {
        const newPost = {
          id: Date.now(),
          likes: 0,
  ...
  const handleLike = async (id) => {
    try {
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL + '/api/posts/${id}/like', { method: 'POST' });
      if (res.ok) {
        fetchPosts();
      } else {
        throw new Error("Server error");
