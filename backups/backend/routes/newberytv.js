...
router.get('/videos', async (req, res) => {
  try {
    const { season, category, tournament, search } = req.query;
...
// POST /api/newberytv/videos (Admin only)
...
// ═══════════════════════════════════════════════════════════════════════════
// 9 & 10. Statistics & Sponsorship tracking
// ═══════════════════════════════════════════════════════════════════════════
...
router.post('/livestreams/:id/sponsors/track', optionalAuthenticate, async (req, res) => {
  const { id } = req.params;
  const { sponsorId, type } = req.body; // type: IMP or CLICK
  try {
    ...
    if (sponsor) {
      ...
      // Also track in general Sponsor module if exists
...
router.get('/statistics', async (req, res) => {
  try {
    ...
    const videoViewsResult = await prisma.video.aggregate({
      _sum: { views: true },
      _avg: { likes: true }
    });
    ...
    // Generate mock graphs / real db summaries
    ...
    res.json({
      ...
      summary: {
        ...
        totalPlaylists,
        totalBroadcasts,
        totalViews: videoViewsResult._sum.views || 0,
        averageLikes: videoViewsResult._avg.likes || 0
      },
      viewerRetention,
      deviceDistribution,
      geographicRetention
